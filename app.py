...
# Removed 'import cv2' as we are no longer using it for stream capture
import json
import random
import time
import threading
...
def control_flash(action):
    ...
    try:
        if action == 'on':
            # FIX: Reduced intensity from 255 to 128 to prevent voltage brown-out
            # Max intensity often crashes the ESP32-CAM due to current draw.
            response = requests.get(f'http://{ESP32_IP}/control?var=led_intensity&val=128', timeout=5)
        elif action == 'off':
...
def capture_esp32_stream():
    ...
    # MJPEG streams often send chunks that start with the boundary, but frames are identifiable by JPEG markers
    ...
    while is_capturing:
        try:
            ...
            if response.status_code == 200:
                print(f"[{datetime.now().strftime('%H:%M:%S')}] Stream connection successful via requests.")
