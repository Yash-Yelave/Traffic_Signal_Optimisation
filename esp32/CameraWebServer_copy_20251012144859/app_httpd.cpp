...
#include "Arduino.h" // Required for Serial, millis, etc.
  ...
  if (buf_len > 1) {
    ...
    if (httpd_req_get_url_query_str(req, buf, buf_len) == ESP_OK) {
      ...
      if (httpd_query_key_value(buf, "time", param, sizeof(param)) == ESP_OK) {
        int time_seconds = atoi(param);
        Serial.printf("Received green light command for %d seconds.\n", time_seconds);

        if (time_seconds > 0) {
          // Set the green light active
...
void startCameraServer() {
  ...
  config.max_uri_handlers = 17; // Increased to 17 for the new handler
  ...
  // NEW: URI for the traffic signal handler
  httpd_uri_t start_green_uri = {
    .uri      = "/start-green",
    .method   = HTTP_GET,
    .handler  = start_green_handler,
    .user_ctx = NULL
  };
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    // Register the new traffic signal handler
    httpd_register_uri_handler(camera_httpd, &start_green_uri);
  }
